...
arr_max = st.sidebar.number_input('Plus grand numéro d\'arrondissement', min_value=arrondissement[0], max_value=arrondissement[len(arrondissement)-1], value=arrondissement[10], step=1)
...
try:
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="age")
    ...
    #(0=sans diplôme; 1=CEP; 2=Diplôme de moniteur indigène; 3=BEPC; 4=Probatoire; 5=BACC; 6=BTS; 7=Licence; 8=Master; 9=DEA; 10=PhD; 11=Autre)
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="diplome")
    st.plotly_chart(fig, use_container_width=True)
    ...
    #df.milieu_residence = np.where(df.milieu_residence==1, "URBAIN", "RURAL")


    sel_mil_res = st.selectbox('Milieu de résidence',options=('TOUT', 'URBAIN', 'RURAL'))

    if (sel_mil_res != "TOUT"):
        oui = "URBAIN" if (sel_mil_res == 1) else "RURAL"
        df = df.loc[df.milieu_residence == oui]

    fig = px.histogram(df, x="arrondissement", y="count",
                color='milieu_residence', barmode='group',
                height=400)
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="etat_matrimonial")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="etat_sante")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="activite_agropastorale")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.scatter(df, x="arrondissement", y="count",
                size="nb_enfant", color="multilangue",
                    hover_name="religion", 
    ...
    fig = px.bar(df, x="arrondissement", y="count", color="type_structure")#, title="la répartition des ménages par arrondissement selon le Type de structure")
    st.plotly_chart(fig, use_container_width=True)
    ...
    if (sel_autochtone != "TOUT"):
        oui = 1 if (sel_autochtone == "OUI") else 2
        df = df.loc[df.autochtone == int(oui)]
    ...
    if sel_occup != "TOUT":
        ...
        df = df.loc[df.statut_occupation == int(occup)]

    fig = go.Figure()

    fig.add_trace(go.Bar(x=df.arrondissement, y=df['count'].values,
                    base= df['count'].values * -1,
                    marker_color='crimson',
                    name='statut de l\'occupation'))
